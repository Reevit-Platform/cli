...
| 003 | Recovery incrementality: holdout, recovered value, honest labelling | backend, frontend | P1 | L | 001 | **GATED** — Step 0 prep done (Q2/Q3/Q5 settled from code). Blocked on **Q1 volume** (needs prod data) and **Q4 consent** (policy). Recommendation: build Step 1 + shadow mode regardless; shadow data answers Q1 with real numbers. |
| 004 | Normalized decline taxonomy + class-aware recovery policy | backend | P2 | L | 003 (shared attribution) | **PARTIAL** — Step 1 code-derived inventory done: 31 free-string codes, 5 alias collisions, 12-class draft registry. **Moolre classifies nothing (0 codes)** — biggest hole, on the primary rail. Still needs the 90-day production export to validate. |
...
| 006 | Claim integrity: deploy freshness, docs contradictions, trust surface | frontend, mintlify-docs, ops | P1 | M | — | **PARTIAL** — Step 1 done: **production is current, and two audit findings are retired** (legal entity, SDK publishing). Docs rate-limits + retention corrected (`3a4b8d4`). **New finding:** the live privacy policy misstates retention in a legal document — escalate to counsel, do not self-edit. Trust Center (Step 6) and counsel engagements (Step 7) remain. |
