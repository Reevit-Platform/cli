...
| 002 | Single-source plan entitlements + delete the dead landing tree | frontend, backend | P1 | M | — | **PARTIAL** — frontend `9a1da2a`, mintlify `3a4b8d4`, backend `493cdc0e`/`bed12a79`: one rate table, GHS base + default, Free webhook limit, copy test, docs rate-limit + retention, Free audit logs, entitlements pinned by test, PRICING.md, routing-experiments entitlement split. **Open:** (a) enterprise `monthly_price_cents` reads as ₵1,000 under the GHS base; (c) webhook/workflow allowances still not modelled in `plans.features`. |
