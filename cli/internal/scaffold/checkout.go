...
var vueScriptSetupPattern = regexp.MustCompile(`(?i)<script\b[^>]*\bsetup\b[^>]*>`)
	...
	_ = filepath.WalkDir(project.Root, func(path string, entry fs.DirEntry, err error) error {
		...
		if entry.IsDir() {
			if path != project.Root {
				switch entry.Name() {
				case "node_modules", ".git", ".next", "dist", "build", "coverage":
...
func checkoutPageScore(project Project, path string) int {
	...
	if strings.Contains(lower, "/components/") {
		return 100
	}
	return 50
}

func validateCheckoutIntegration(project Project, target Target) (string, error) {
	if target.Checkout == nil || strings.TrimSpace(target.Checkout.PagePath) == "" {
		return "", nil
	}

	pagePath, err := projectPath(project.Root, target.Checkout.PagePath)
	if err != nil {
		return "", err
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return "", fmt.Errorf("checkout page %q does not exist", target.Checkout.PagePath)
		}
		return "", fmt.Errorf("inspect checkout page: %w", err)
	}
	if info.IsDir() {
		return "", fmt.Errorf("checkout page %q is a directory", target.Checkout.PagePath)
	}
	if len(target.Files) != 1 {
		return "", fmt.Errorf("checkout target must generate exactly one component")
	}

	for _, componentRel := range target.Files {
		componentPath, pathErr := projectPath(project.Root, componentRel)
		if pathErr != nil {
			return "", pathErr
		}
...
func configuredCheckoutTarget(project Project, target Target) Target {
	if target.Checkout == nil || (len(target.Checkout.Fields) == 0 && len(target.Checkout.MetadataFields) == 0) {
		return target
	}
	for templateName, componentRel := range target.Files {
		componentPath, err := projectPath(project.Root, componentRel)
		if err != nil {
			return target
		}
		existing, err := os.ReadFile(componentPath)
		if err != nil {
			return target
		}
		configID := checkoutConfigID(target.Checkout)
		if strings.Contains(string(existing), "reevit-checkout-config: "+configID) {
			return target
		}

		ext := filepath.Ext(componentRel)
		configuredRel := strings.TrimSuffix(componentRel, ext) + ".reevit-" + configID + ext
		target.Files = map[string]string{templateName: configuredRel}
		return target
	}
	return target
}
...
func projectPath(root, rel string) (string, error) {
	...
	within, err := filepath.Rel(rootAbs, path)
	if err != nil || within == ".." || strings.HasPrefix(within, ".."+string(filepath.Separator)) {
		return "", fmt.Errorf("path %q must stay inside the project", rel)
	}
	return path, nil
}

func injectCheckout(project Project, target Target, pagePath string) (bool, error) {
	raw, err := os.ReadFile(pagePath)
	if err != nil {
		return false, fmt.Errorf("read checkout page: %w", err)
	}
	content := string(raw)
	originalContent := content
	hasInsertion := strings.Contains(content, "reevit-checkout:start")

	var componentRel string
	for _, componentRel = range target.Files {
		break
	}
	componentPath, err := projectPath(project.Root, componentRel)
	...
	importPath, err := filepath.Rel(filepath.Dir(pagePath), componentPath)
	if err != nil {
		return false, fmt.Errorf("resolve checkout import: %w", err)
	}
	...
	case StackNext, StackReact:
		...
		if hasInsertion {
			content, err = replaceMarkedImport(content, fmt.Sprintf("import { ReevitCheckoutButton } from %q; // reevit-checkout:import\n", importPath))
		} else {
			content, err = injectReactCheckout(content, importPath)
		}
	case StackVue:
		if hasInsertion {
			content, err = replaceMarkedImport(content, fmt.Sprintf("import ReevitCheckoutButton from %q; // reevit-checkout:import\n", importPath))
		} else {
			content, err = injectVueCheckout(content, importPath)
		}
	case StackSvelte:
		if hasInsertion {
			content, err = replaceMarkedImport(content, fmt.Sprintf("  import ReevitCheckoutButton from %q; // reevit-checkout:import\n", importPath))
		} else {
			content, err = injectSvelteCheckout(content, importPath)
		}
	default:
	...
	info, err := os.Stat(pagePath)
	if err != nil {
		return false, fmt.Errorf("inspect checkout page: %w", err)
	}
	if err := os.WriteFile(pagePath, []byte(content), info.Mode().Perm()); err != nil {
		return false, fmt.Errorf("update checkout page: %w", err)
	}
	return true, nil
}

func replaceMarkedImport(content, importLine string) (string, error) {
	markerAt := strings.Index(content, "reevit-checkout:import")
	if markerAt < 0 {
		return "", fmt.Errorf("checkout page has an insertion block but no managed import marker")
	}
	lineStart := strings.LastIndex(content[:markerAt], "\n") + 1
	lineEndOffset := strings.Index(content[markerAt:], "\n")
	lineEnd := len(content)
	if lineEndOffset >= 0 {
		lineEnd = markerAt + lineEndOffset + 1
	}
	return content[:lineStart] + importLine + content[lineEnd:], nil
}

func injectReactCheckout(content, importPath string) (string, error) {
	importLine := fmt.Sprintf("import { ReevitCheckoutButton } from %q; // reevit-checkout:import\n", importPath)
	insertAt := 0
	...
	if selfClosing {
		match := []int{rootStart, closeAt}
		lineStart := strings.LastIndex(content[:match[0]], "\n") + 1
		indent := content[lineStart:match[0]]
		if strings.TrimSpace(indent) != "" {
			indent = ""
		}
		root := content[match[0]:match[1]]
		replacement := "<>\n" +
			indent + "  " + root + "\n" +
			indent + "  {/* reevit-checkout:start */}\n" +
			indent + "  <ReevitCheckoutButton amount={5000} />\n" +
			indent + "  {/* reevit-checkout:end */}\n" +
			indent + "</>"
	...
	block := indent + "{/* reevit-checkout:start */}\n" +
		indent + "<ReevitCheckoutButton amount={5000} />\n" +
		indent + "{/* reevit-checkout:end */}\n"
	return content[:lineStart] + block + content[lineStart:], nil
}

// findDefaultExportJSXRoot returns the exported page component's root JSX
// boundaries. Constraining insertion to this root avoids modifying helper
// components that happen to appear later in the same file.
...
func findFunctionBody(content string, functionAt int) int {
	...
	// A return type can itself be an object: `(): { ok: boolean } { ... }`.
	// Only treat an immediately opening brace after ':' as a type brace;
	// otherwise the next brace starts the function body (`(): JSX.Element {`).
	i++
	for i < len(content) && (content[i] == ' ' || content[i] == '\t' || content[i] == '\r' || content[i] == '\n') {
		i++
	}
